...
*/
    ...
    s->top=-1;      //natavenie vrcholu zásobníka
}
...
** funkci stackEmpty.
*/
    if (stackEmpty(s)){         //overenie zaplnenia zásobníka
        stackError(SERR_TOP);
        return;
    }
    *c = s->arr[s->top];        //do c vloží symbol z vrcholu zásobníka
}
...
** funkci stackFull.
*/
    if (stackFull(s)){          //overenie zaplnenia zásobníka
        stackError(SERR_PUSH);
        return;
    }
    s->top++;                   //zvýšenie vrcholu
    s->arr[s->top] = c;         //pridanie prvku na nový vrchol
	}
